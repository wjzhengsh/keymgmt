...
2. Run the following cURL command to view general characteristics about your keys.
    ...
    If you need to browse keys within a specified Cloud Foundry org and space in your account, replace `Bluemix-Instance` with the appropriate `Bluemix-org` and `Bluemix-space` headers. See [Forming an API Request](/docs/services/keymgmt/keyprotect_authentication#form_api_request) for code samples.
    {: tip}
